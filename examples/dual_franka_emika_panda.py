...
def define_waypoints(data, site_ref_left, site_ref_right, target_quat_left, target_quat_right):
...
def check_reached(meas_left_pos, tgt_left_pos, meas_left_quat, tgt_left_quat,
                  meas_right_pos, tgt_right_pos, meas_right_quat, tgt_right_quat,
    ...
    return (err_pos_left <= pos_threshold and 
            err_ori_left <= ori_threshold and
            err_pos_right <= pos_threshold and 
            err_ori_right <= ori_threshold)
...
def ik_iteration(configuration, tasks, dt, solver, data, model,
    ...
    return check_reached(measured_left_pos, left_target_pos, measured_left_quat, left_target_quat,
                         measured_right_pos, right_target_pos, measured_right_quat, right_target_quat,
                         pos_threshold, ori_threshold)
...
def run_waypoints(model, data, configuration, viewer, left_waypoints, right_waypoints,
                  left_ee_task, right_ee_task, tasks, solver,
                  site_left_id, site_right_id, site_ref_left, site_ref_right,
                  pos_threshold, ori_threshold, max_iters, rate):
    """Plans and executes each waypoint until reached."""
    for wp_index, (left_wp, right_wp) in enumerate(zip(left_waypoints, right_waypoints)):
        ...
        while not reached and viewer.is_running():
            ...
            update_mocap_targets(model, data, current_left_pos, current_left_quat,
            ...
            for _ in range(max_iters):
                reached = ik_iteration(configuration, tasks, rate.dt, solver, data, model,
                                       site_left_id, site_right_id,
...
def main():
    ...
    with mujoco.viewer.launch_passive(model=model, data=data, show_left_ui=False, show_right_ui=False) as viewer:
        ...
        # Execute waypoints
        run_waypoints(
            model, data, configuration, viewer,
        ...
        for _ in range(max_iters):
            ik_iteration(configuration, tasks, rate.dt, solver, data, model,
                         site_left_id, site_right_id,
                         lifted_left_pos, tgt_quat_left,
                         lifted_right_pos, tgt_quat_right,
                         pos_threshold, ori_threshold,
                         gripper_val=0)
            viewer.sync()
            rate.sleep()
